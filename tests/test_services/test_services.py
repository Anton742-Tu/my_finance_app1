...
def sample_transactions() -> List[Transaction]:
    return [
        Transaction(
            date="2024-01-15", amount=-1000.0, category="Супермаркеты", description="Пятерочка", cashback=50.0
        ),
        Transaction(
            date="2024-01-20",
            amount=-500.0,
        ...
        Transaction(date="2024-02-01", amount=-200.0, category="Переводы", description="Иван С.", cashback=0.0),
    ]


def test_analyze_cashback_categories(sample_transactions: List[Transaction]) -> None:
...
def test_investment_bank() -> None:
    ...
    transactions = [
        {"date": "2024-01-15", "amount": -123.0, "category": "", "description": "", "cashback": 0.0},
        {"date": "2024-01-20", "amount": -477.0, "category": "", "description": "", "cashback": 0.0},
    ]
    result = investment_bank("2024-01", transactions, 100)
    assert result == 100  # (200-123) + (500-477) = 77 + 23 = 100


def test_simple_search(sample_transactions: List[Transaction]) -> None:
...
def test_find_phone_transactions(sample_transactions: List[Transaction]) -> None:
...
def test_find_person_transfers(sample_transactions: List[Transaction]) -> None:
