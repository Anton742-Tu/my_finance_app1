...
def test_get_home_data(mock_stocks, mock_currency, mock_top, mock_cashback, mock_analyze, mock_load, mock_operation):
    """Тест получения данных для главной страницы"""
    # Настраиваем моки
    mock_load.return_value = [mock_operation]
    mock_analyze.return_value = {"total_spent": 100.00, "by_category": {"Food": 100.00}}
    mock_cashback.return_value = 1.00
    ...
    assert result["cashback"] == 1.00
    assert len(result["top_transactions"]) == 1
    assert result["top_transactions"][0].amount == 100.00
