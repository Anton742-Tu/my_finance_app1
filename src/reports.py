...
def generate_excel_report(transactions: List[dict], output_path: str):
    ...
    with pd.ExcelWriter(output_path, engine="xlsxwriter") as writer:
        ...
        if len(df) > 0:
            pivot_table = df.pivot_table(index="Категория", values="Сумма операции", aggfunc="sum").reset_index()

            pivot_table.to_excel(writer, sheet_name="Сводка", index=False, startrow=3)
